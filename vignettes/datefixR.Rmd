...
package consisting of a single user-accessible function, `fix_dates()`, which
converts dates written in different formats into R's built-in `Date` class.
...
consider a dataframe with two columns of dates in differing formats
...
`fix_dates()` requires two arguments, `df`, a dataframe (or tibble) object and
`col.names`, a character vector containing the names of columns to be
standardized. By default, the first column of the dataframe is assumed to contain
