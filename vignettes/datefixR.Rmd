...
`fix_date_df()` requires two arguments, `df`, a data frame (or tibble) object and
...
```{R}
fixed.dates <- fix_date_df(bad.dates,
...
```{R}
imputed.dates <- fix_date_df(impute.dates, "some.date")
...
```{R}
imputed.dates <- fix_date_df(impute.dates,
                             "some.date",
                             day.impute = 1,
                             month.impute = 1)
knitr::kable(imputed.dates)
...
`day.impute` and `month.impute` can also be passed to `fix_date_char()` for
similar functionality. 

```{R}
fix_date_char("2014", day.impute = 1, month.impute = 1)
```
